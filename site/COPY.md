# Texto do site da Inbre — fonte de edição

Este arquivo é a fonte do texto. Editou aqui, reflita no `index.html` (e vice-versa).
...
**Grade de contato** — ainda não publicada. O markup está pronto, em comentário, no fim da
seção de contato do `index.html`: é descomentar e preencher. É o bloco que quem veio checar
se a Inbre é real vem procurar, então nada aqui pode ser inventado.

- [NÚMERO VISÍVEL]
- @[INSTAGRAM]
- [EMAIL]
- CNPJ [CNPJ] — opcional, mas é o item que mais pesa. Se não houver, apague a linha inteira
...
Cada posição manda uma mensagem diferente, para você saber de onde a pessoa saiu.
