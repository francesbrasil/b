...
export default defineConfig({
	...
	server: {
		port: 3000,
		// Na hospedagem do Horizons existe um proxy de borda que mapeia
		// /hcgi/api para a API e /hcgi/platform para o PocketBase. Fora dali
		// esses dois caminhos simplesmente não existem: o formulário de
		// diagnóstico e o botão "Contratar plano" respondem 404 e a página
		// mostra só "tente novamente em alguns instantes", que não diz nada.
		// Isto reproduz o mesmo mapeamento no `npm run dev`, para o site rodar
		// inteiro fora do Horizons. Em produção quem faz este papel é o
		// servidor da frente (Nginx, Caddy, o proxy do provedor) — ver o README.
		proxy: {
			'/hcgi/api': {
				target: process.env.API_URL || 'http://127.0.0.1:3001',
				changeOrigin: true,
				rewrite: (caminho) => caminho.replace(/^\/hcgi\/api/, ''),
			},
			'/hcgi/platform': {
				target: process.env.POCKETBASE_URL || 'http://127.0.0.1:8090',
				changeOrigin: true,
				rewrite: (caminho) => caminho.replace(/^\/hcgi\/platform/, ''),
			},
		},
